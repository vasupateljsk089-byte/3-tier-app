...
#   key_name      = var.key_name
  ...
  tag_specifications {
    ...
    tags = merge(
      var.tags,
      {
        Name = "${var.environment}-${var.project_name}-frontend-volume"
      }
    )
  }
}
...
resource "aws_autoscaling_group" "frontend" {
  name                      = "${var.environment}-${var.project_name}-frontend-asg"
  min_size                  = var.min_size
  max_size                  = var.max_size
  desired_capacity          = var.desired_capacity
  vpc_zone_identifier       = var.subnet_ids # select subnet
  ...
  tag {
    key                 = "Name"
    value               = "${var.environment}-${var.project_name}-frontend"
...
resource "aws_autoscaling_policy" "frontend_cpu" {
  name                   = "${var.environment}-${var.project_name}-frontend-cpu-tracking"
...
resource "aws_cloudwatch_metric_alarm" "frontend_high_cpu" {
  alarm_name          = "${var.environment}-${var.project_name}-frontend-high-cpu"
...
resource "aws_cloudwatch_metric_alarm" "frontend_unhealthy" {
  alarm_name          = "${var.environment}-${var.project_name}-frontend-unhealthy-hosts"
  comparison_operator = "GreaterThanThreshold"
